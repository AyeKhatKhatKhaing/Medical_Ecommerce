...
class SubCategoryRepo
{
    public function getSubCategory($request)
    {
        ...
        $datatables       = DataTables::of($sub_category)
            ...
            ->addColumn('no', function ($sub_category) {
                ...
                $img = isset($sub_category->img) ? $sub_category->img : asset('/backend/media/blank-image.svg');
            ...
            })->editColumn('sort_by', function ($sub_category) {
                $isDisabled = '';
                if($sub_category->is_home == false){
                    $isDisabled = 'disabled';
                }
                $input = "";
                $input     .= '<div class="">';
                $route    = "'" . route('subcate.sortBy') . "'";
                $input .= '<input data-id="' . $sub_category->id . '" value="' . $sub_category->sort_by . '"  onkeyup="sortByKeyUp(' . $sub_category->id . ',' . $route . ')"
                            class="form-control" type="number" id="sub_cate_id'. $sub_category->id .'" '.$isDisabled.'>';
                $input .= '</div>';
                ...
                if($checkAdminRole == true){
                    ...
                    <a href="' . route('sub-category.edit', $category->id) . '" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
                    ...
                    <form action="' . route('sub-category.destroy', $category->id) . '" method="POST" class="action-form" style="display:inline">
            ...
            ->rawColumns(['name', 'status', 'image', 'is_published', 'updated_at', 'sort_by', 'action']);
    ...
    {
        $data     = DB::table('sub_categories')->orderBy('id', 'DESC')->whereNull('sub_categories.deleted_at');

        $search = $request->search;
        $status = $request->status;

        if (isset($search)) {
            $data = $data->where('name_en', 'LIKE', "%$search%")->latest();
        }
    ...
    public function saveSubCategory($request, $id = null)
    {
        ...
        $input['is_home']  = $request->is_home == 'on' ? 1 : 0;
