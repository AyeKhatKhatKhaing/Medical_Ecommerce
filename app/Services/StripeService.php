    ...
    function payment($cardinfo,$checkoutItems) {
        ...
        // try {
            ...
            // try { 
                ...
                if($checkoutItems['currentOrderId'] == 0) {
                    $order = $order->saveCheckoutOrders($checkoutItems);
                }else{
                    $order = Order::find($checkoutItems['currentOrderId']);
                    $order->payment_status = $checkoutItems['cardType'] == 'other_pay' ? 2 : 1;
                    $order->payment_type = $checkoutItems['cardType'];
                    $order->save();
                    $orderItems = OrderItems::where("orders_id",$checkoutItems['currentOrderId'])->get();
