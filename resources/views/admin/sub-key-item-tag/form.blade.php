<div class="row">
    <div class="row mb-5">
        <div class="col-md-8">
            <h3 class="card-title">
                @if($formMode == "edit") Edit Sub Key Item @else Add New Sub Key Item @endif
            </h3>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                <div class="float-end">
                    ...
                    <a href="{{ url('admin/sub-key-item-tag') }}" title="Back"><button type="button" class="btn btn-secondary btn-sm cancel"><i class="bi bi-x" aria-hidden="true"></i> Cancel</button></a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card-body">
                <div class="card-body content-body">
                    <div class="tab-content">
                        <div class="form-group mb-5{{ $errors->has('key_item_tag') ? 'has-error' : ''}}">
                            {!! Form::label('key_item_tag_id', 'Key Item Tag', ['class' => 'form-label required']) !!}
                            {!! Form::select('key_item_tag_id', $key_item_tags, null , ['class' => 'form-select form-select-solid', 'data-control' => 'select2', 'data-hide-search' => "true", 'data-placeholder' => "Select Key Item Tag", 'placeholder' => "Select Key Item Tag", "data-kt-ecommerce-product-filter" => "key_item_tag_id"]) !!}
                            {!! $errors->first('key_item_tag_id', '<p class="help-block text-danger">:message</p>') !!}
                        </div>
                            ...
                            <div class="tab-pane fade {{ $attr == 'en' ? 'active show' : '' }}"  id="{{ strtolower($lngcode) }}-tab">
                                ...
                                @if($attr == 'tc')
                                    <div class="form-check mb-5 mt-5 d-flex align-items-center">
                                        <input class="form-check-input" {{isset($sub_key_item_tag->is_translate)&&$sub_key_item_tag->is_translate==1?'checked':''}} type="checkbox" name="is_translate" id="auto_translate" value="1">
