<div class="row">
    <div class="row mb-5">
        <div class="col-md-8">
            <h3 class="card-title">
                @if($formMode == "edit") Edit Time Slot Tag @else Add New Time Slot Tag @endif
            </h3>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                <div class="float-end">
                    ...
                    <a href="{{ url('admin/time-slot-tag') }}" title="Back"><button type="button" class="btn btn-secondary btn-sm cancel"><i class="bi bi-x" aria-hidden="true"></i> Cancel</button></a>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="tab-content">
                        @foreach (config('lng.attr') as $lngcode => $attr)
                            <div class="tab-pane fade {{ $attr == 'en' ? 'active show' : '' }}"  id="{{ strtolower($lngcode) }}-tab">
                                ...
                                <div class="form-group mb-5{{ $errors->has('remind_notice_'.$attr) ? 'has-error' : ''}}">

                                    {!! Form::label('remind_notice_'.$attr, 'Remind' . ' (' . $lngcode . ')', ['class' => 'form-label']) !!}
                                    {!! Form::textarea('remind_notice_'.$attr, null, ('required' == 'required') ? ['class' => 'form-control editor'] : ['class' => 'form-control editor']) !!}
                                    {!! $errors->first('remind_notice_' . $attr, '<p class="help-block text-danger">:message</p>') !!}
                                </div>

                                @if($attr == 'tc')
                                    <div class="form-check mb-5 mt-5 d-flex align-items-center">
                                        <input class="form-check-input" type="checkbox" {{isset($time_slot_tag->is_translate)&&$time_slot_tag->is_translate==1?'checked':''}} name="is_translate" id="auto_translate" value="1">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="panel-block">
                    <div class="form-group">
                        <div id="holder-img">
                            @if(!empty($time_slot_tag->img))
                                <div class='lfmimage-container imglfmc0'>
                                    <img src="{{ asset($time_slot_tag->img) }}" class='lfmimage w-100' style="height: 20rem;">
                                    <div>
                        ...
                        <div class="input-group mt-3">
                            ...
                            <input id="img" class="form-control" type="text" name="img" value="{{isset($time_slot_tag) ? $time_slot_tag->img : ''}}">
                        </div>
                    </div>
                </div>
            </div>
