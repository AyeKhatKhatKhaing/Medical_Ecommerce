<div class="row">
    <div class="row mb-5">
        <div class="col-md-8">
            <h3 class="card-title">
            @if($formMode == "edit") Edit Main Optional Item @else Add New Main Optional Item @endif
            </h3>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                <div class="float-end">
                    ...
                    <a href="{{ url('/admin/main-optional-item') }}" title="Back"><button type="button" class="btn btn-secondary btn-sm cancel"><i class="bi bi-x" aria-hidden="true"></i> Cancel</button></a>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="card-body content-body">
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade {{ $attr == 'en' ? 'active show' : '' }}"  id="{{ strtolower($lngcode) }}-tab">
                            ...
                            <div class="form-group mb-5{{ $errors->has('remind_message_'.$attr) ? 'has-error' : ''}}">
                                {!! Form::label('remind_message_'.$attr, 'Remind Message' . ' (' . $lngcode . ')', ['class' => 'form-label']) !!}
                                {!! Form::textarea('remind_message_'.$attr, null, ('required' == 'required') ? ['class' => 'form-control editor'] : ['class' => 'form-control editor']) !!}
                                {!! $errors->first('remind_message_'.$attr, '<p class="help-block text-danger">:message</p>') !!}
                            </div>
                            @if($attr == 'tc')
                                <div class="form-check mb-5 mt-5 d-flex align-items-center">
                                    <input class="form-check-input" type="checkbox" {{isset($mainoptionalitem->is_translate)&&$mainoptionalitem->is_translate==1?'checked':''}} name="is_translate" id="auto_translate" value="1">
