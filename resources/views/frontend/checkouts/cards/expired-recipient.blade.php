@if($is_two_person_plan == false)
@foreach($recipients as $rkey => $recipient)
@php
...
@endphp
@if(($isunavailable == false && $recipient->date != null && $recipient->date < date('Y-m-d')) || ($isunavailable == false && $recipient->date != null && $recipient->date <= date('Y-m-d')))
<input type="hidden" value="yes" class="is_expired"> 
...
<div>
    ...
    <div class="">
        <div class="flex justify-between">
            <p class="font-bold me-body-26 text-darkgray">{{__('labels.check_out.expired')}}</p>
            <form action="" id="removeRecipient{{$recipient->id}}" style="display: flex;
    align-items: center;">
                <input type="hidden" name="recipientId" value="{{$recipient->id}}">
                <input type="hidden" name="productId" value="{{$recipient->product->id}}">
                <div class="flex">
                    <p class="italic me-body-16 font-normal text-mered mr-8">{{__('labels.check_out.product_expired')}}</p>
    ...
    <div class="hidden">
        <div class="flex justify-between">
            ...
            <div class="flex flex-wrap">
                ...
                <p data-id="5" class="underline me-body-16 text-darkgray remove-btn cursor-pointer">Remove</p>
            </div>
        </div>
    </div>
</div>
...
($row[1]->date  != null && $row[1]->date > date('Y-m-d') && $row[0]->date != null && $row[0]->date < date('Y-m-d')) ||
($row[1]->date  != null && $row[1]->date < date('Y-m-d') && $row[0]->date != null && $row[0]->date > date('Y-m-d'))  ||
...
<div>
    ...
    <div class="">
        <div class="flex justify-between">
            <p class="font-bold me-body-26 text-darkgray">{{__('labels.check_out.expired')}}</p>
            <form action="" id="removeRecipient{{$keyVal->product->id}}{{$keyVal->id}}" style="display: flex;align-items: center;">
            ...
            <!-- <p data-id="1" class="underline me-body-16 text-darkgray remove-btn cursor-pointer">Remove
            </p> -->
        </div>
    </div>
    <div class="hidden">
        ...
        <div class="flex sm:flex-row flex-col available-card">
            <div class="w-[100px] mr-5">
                <img src="{{$keyVal->product->merchant->icon}}" alt="logo"
                    class="mr-5 rounded-full max-w-[100x] max-h-[100px] object-cover checkout-card-logo-img checkout_product_image" />
            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <div component-name="me-checkout-merchant-recipient-cancerMarker-detail" class="mt-1">
                                    ...
                                    <div
                                        ...
                                        <div class="flex justify-between w-full">
                                            <p data-id="receipientData{{$group->id}}{{$recipient->id}}" data-label="{{__('labels.product_detail.decide_later')}}"
                                                class="custom-cancer-markers-remark-title cursor-pointer flex flex-wrap font-normal me-body-18 text-darkgray underline">
