<div id="compare-modal{{ $product->id }}-quick-preview"
    ...
    <div class="flex justify-between items-center compare-two-col">
        <div class="flex gap-[24px] xl:gap-[35px] w-full max-w-[90%] xl:max-w-[65%] selected-items"
            data-id={{ $product->id }}>
            <div data-id="add-compare-modal{{ $product->id }}-quick-preview"
                data-parent-modal-id="compare-modal{{ $product->id }}-quick-preview"
        ...
        <div class="w-fit flex justify-between xl:gap-[50px] compare-textbox">
            <div class="compare-text-section text-center">
                <button type="button" data-id="{{ $product->id }}"
                    class="11 btn-compare border border-darkgray py-[10px] px-5 rounded-full text-darkgray bg-whitez font-bold font-secondary me-body16 hover:border-blueMediq hover:bg-blueMediq hover:text-whitez">@lang('labels.footer.compare')</button>
                <p class="pt-1 font-bold font-secondary me-body15 text-darkgray">@lang('labels.footer.compare_products')</p>
            </div>
            <p data-id="compare-modal{{ $product->id }}-quick-preview"
...
<div id="add-compare-modal{{ $product->id }}-quick-preview"
    class="modal flex hidden items-center justify-center fixed z-[1001] left-0 top-0 w-full h-full bg-black/[.75] overflow-hidden">
    <!-- Modal content -->
    <div class="modal-content max-w-[80%] lg:max-w-[578px] shadow-shadow02 my-[15%] mx-auto">
        <div class="relative rounded-[12px] bg-white">
            <div class="flex ml-auto mr-6 w-5 h-5 absolute right-0 top-4">
                <span data-id="add-compare-modal{{ $product->id }}-quick-preview"
            ...
            <div class="modal-content-plan">
                ...
                <div class="bg-white p-10 pt-5 pr-[24px] rounded-b-[12px]">
                    <p
                        class="helvetica-normal text-darkgray me-body16 recently-or-recommended-text{{ $product->id }}">
                        @lang('labels.product.recent_view')</p>
                    <div class="flex flex-col compare-box-container suggestion-product-list{{ $product->id }}">
                    </div>
                </div>
            </div>
        </div>
