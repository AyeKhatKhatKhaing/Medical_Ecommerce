    ...
    <main>
        <div class="blog-gallery-container boc-container py-[30px] lg:pt-[60px] lg:pb-[90px]">
            <div class="flex flex-wrap lg-custom:flex-nowrap justify-between mt-[12px]">
                <div class="blog-left">
                            ...
                            <div component-name="me-blog-detail-top" class="me-blog-detail-top-container">
                                <div class="mbdt--title--container mb-5">
                                    <div class="flex items-baseline justify-start">
                                        <p
                                            class="mbdt--title--feature helvetica-normal text-whitez me-body16 px-[20px] rounded-[50px] flex items-center justify-center bg-blueMediq w-fit">
                                            {{ @langbind($blogDetails->blogCategory, 'name') }}
                                        </p>
                                        <button id="mobile-summary"
                                            class="helvetica-normal text-orangeMediq me-body18 border border-whitez bg-white py-[4px] px-[10px] items-center justify-center ml-auto">@lang('labels.check_out.summary')</button>
                                    </div>
                                    <h1
                                        class="mbdt--title--name helvetica-normal text-blueMediq me-title48 font-bold leading-[57.6px] my-[20px]">
                                        {{ @langbind($blogDetails, 'title') }}
                                    </h1>
                                    <p class="mbdt--title--author helvetica-normal text-darkgray me-body16 font-bold ">
                                        {{ @langbind($blogDetails, 'name') }}
                                    </p>
                                    <p class="mbdt--title--author helvetica-normal text-darkgray me-body16 font-bold ">
                                        {{ @langbind($blogDetails->blogAuthor, 'name') }},
                                    </p>
                                    <p class="mbdt--title--date helvetica-normal me-body16 text-lightgray">
                                        @lang('labels.last_updated')    @if(lang() == "en")
                                ...
                                <div class="mbdt--des--container mb-10 text-darkgray me-body26 leading-[41.6px]">
                                    <p>
                                        {!! @langbind($blogDetails, 'desc') !!}
                                    </p>
                                </div>
                                <div class="mbdt--img-container rounded-[20px] mb-10" alt="{{ $blogDetails->main_image_alt_text }}"
                                    style="background-image:url('{{ $blogDetails->main_image_url }}');background-size: cover;background-repeat:no-repeat;">
                                    <div class="bg-lightgray opacity-30 h-[400px] rounded-[20px]"></div>
                                </div>
                            </div>
                        </div>
                        {{-- @foreach ($blogDetails->blogDetails as $data)
                        @if ($data->title_no_en == 1)
                        <div id="no1">
                            <div component-name="me-blog-video-text" class="mbvt-container" >
                                <div class="mbvt-title me-body36 helvetica-normal text-darkgray font-bold mb-5">
                                {{@langbind($data,'title')}}
                                </div>
                                <div class="mbvt-iframe">
                                    <iframe class="w-full rounded-[20px] half-aspect"
                                        src="{{$data->video_url}}" title="#"
                                        frameborder="0"
                                ...
                                <a href="{{$data->video_url}}" class="mbvt-video-link flex items-center justify-start helvetica-normal text-darkgray me-body18 font-bold my-4">
                                    <img src="{{asset('frontend/img/main-triangle.svg')}}" alt="blue triangle icon" class="blue-triangle-icon">
                                    Video source from Youtube
                                </a>


                                <div class="mbdt--des--container mb-10 text-darkgray me-body23 leading-[36.8px]">
                                    {!! @langbind($data,"desc") !!}
                                </div>
                            </div>
                        </div>
                    ...
                    {{-- {{ $data->title_no_en}}
                        ...
                        $title = "";
                        $sort_order_no "";
                        if($data->title_no_en == 1 || $data->title_no_en == 14){
                            if($data->title_no_en == 1) {
                        ...
                        @if ($data->title_no_en == 1)
                            {{-- @php 
                              var_dump($blogDetails->blogDetails[12]);
                            @endphp --}}
                            <div id="no1">
                                <div component-name="me-blog-video-text" class="mbvt-container">
                                    <h3 class="mbvt-title me-body36 helvetica-normal text-darkgray font-bold mb-5">
                                        {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                                    </h3>
                                    @if(isset($blogDetails->blogDetails[12]) && $blogDetails->blogDetails[12]->title_no_en == 13)
                                    <div class="mbvt-iframe">
                                        <iframe class="w-full rounded-[20px] half-aspect" src="{{ $blogDetails->blogDetails[12]->video_url }}"
                                            title="#" frameborder="0"
                                            allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                                            allowfullscreen="">
                                        </iframe>
                                    </div>
                                    <a href="{{ $blogDetails->blogDetails[12]->video_url }}"
                                        class="mbvt-video-link flex items-center justify-start helvetica-normal text-darkgray me-body18 font-bold my-4">
                                        <img src="{{ asset('frontend/img/main-triangle.svg') }}" alt="blue triangle icon"
                                            class="blue-triangle-icon">
                                        {!! @langbind($blogDetails->blogDetails[12], 'desc') !!}
                                    </a>
                                    @endif
                        ...
                        @endif
                        @if ($data->title_no_en == 2)
                            <div class="no2-section" id="no{{ $data->title_no_en }}">
                                <div component-name="me-quote-image" class="me-quote-image-container">
                                    <div class="mqi-container">
                                        <h3 class="mbvt-title me-body36 helvetica-normal text-darkgray font-bold mb-5">
                                            {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                                        </h3>
                                        @php
                                            $blogImages = App\Models\BlogImage::where('blog_id', $data->blog_id)
                                                ->where('blog_details_no', $data->title_no_en)
                                                ->first();
                                            $image_gallery_alt_text = explode(',',$data->image_gallery_alt_text);
                                        @endphp
                                        <div class="flex items-center justify-between">
                                            @if ($blogImages->img != null)
                                                @foreach (json_decode($blogImages->img) as $img)
                                                    @if ($loop->index == 2)
                                                        @break
                                                    @endif
                                                <div class="basis-[49%] rounded-[20px]">
                                                    <img src="{{ $img }}" alt="{{array_key_exists($loop->index,$image_gallery_alt_text)?$image_gallery_alt_text[$loop->index]:''}}"
                                                        data-id={{ $loop->index }}
                                                        class="blog-thumnail cursor-pointer object-cover w-full h-full">
                                                </div>
                                                @endforeach
                                            @endif
                                        </div>

                                    <dialog component-name="me-blog-image-zoom-popup"
                                        class="mbizoom-container fixed z-[1001] left-0 top-0 w-full h-full overflow-hidden bg-linear-black items-center justify-center hidden">
                                        <div
                                            class="w-full max-w-[1150px] shadow-shadow02 my-[15%] rounded-[12px] text-darkgray relative mx-auto">
                                            <div class="flex items-center justify-between md:mb-10 mb-6">
                                                <button
                                                    class="lr-popup-close absolute z-[1001] top-[56px] right-[90px]">
                                                    <img src="{{ asset('frontend/img/lr_round-close.svg') }}"
                                                        alt="close icon" class="mbizoom-close">
                                                </button>
                                            </div>
                                            <div class="mbizoom-slider w-full">
                                                @if ($blogImages->img != null)
                                                    @foreach (json_decode($blogImages->img) as $img)
                                                        {{-- <div class="images{{$loop->index}}"> --}}
                                                        <div
                                                            class="showimage px-[70px] mx-auto w-full max-w-[1150px] h-[640px] rounded-[12px] ">
                                                            <img src="{{ $img }}"
                                                                class="blog-thumnail w-full h-full rounded-[12px] object-cover desktop-img">
                                                            <img src="{{ $img }}"
                                                                class="blog-thumnail w-full h-full rounded-[12px] object-cover mobile-img">
                                                        </div>
                                                        {{-- </div> --}}
                                                    @endforeach
                                                @endif
                                    ...
                                    {!! @langbind($data, 'desc') !!}
                                    <button class="rounded-[50px] px-[20px] py-[10px] border border-darkgray bg-whitez flex items-center justify-center me-body16 text-darkgray helvetica-normal font-bold mt-5 hover:text-whitez hover:bg-blueMediq hover:border-blueMediq">Go
                                        to 1800+ Female Health Check</button>
                                </div>
                            </div>
                        </div>
                    @endif
                    @if ($data->title_no_en == 3)
                        <div class="no3-section" id="no{{ $data->title_no_en }}">
                            <div component-name="me-blog-only-title" class="mbotitle mt-[32px] mb-5">
                                <h3
                                    class="mbo-title helvetica-normal me-body36 font-bold text-darkgray leading-[43.2px]">
                                    {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                                </h3>
                            </div>
                            <div component-name="me-blog-three-images-card" class="mbtic-container">
                                <div class="flex items-center justify-between gap-5 h-[400px] mbtic-items-container">
                                    @php
                                        $blogImages = App\Models\BlogImage::where('blog_id', $data->blog_id)
                                            ->where('blog_details_no', $data->title_no_en)
                                            ->first();
                                    ...
                                    @if ($blogImages->img != null)
                                        ...
                                        @endif
                                        <div class="item">
                                            <img src="{{ $img }}" data-id="1" alt="{{array_key_exists($loop->index,$image_gallery_alt_text)?$image_gallery_alt_text[$loop->index]:''}}"
                                                class="blog-thumnail cursor-pointer w-full h-full object-cover rounded-[12px]">
                                        </div>
                                    @endforeach
                                @endif
                            ...
                            <dialog component-name="me-blog-image-zoom-popup"
                                class="hidden mbizoom-container fixed z-[1001] left-0 top-0 w-full h-full overflow-hidden bg-linear-black items-center justify-center">
                                <div
                                    class="w-full max-w-[1150px] shadow-shadow02 my-[15%] rounded-[12px] text-darkgray relative mx-auto">
                                    <div class="flex items-center justify-between md:mb-10 mb-6">
                                            ...
                                            @foreach (json_decode($blogImages->img) as $img)
                                                <div class="images{{ $loop->index }}">
                                                    <div
                                                        class="showimage px-[70px] mx-auto w-full max-w-[1150px] h-[640px] rounded-[12px] ">
                                                        <img src="{{ $img }}"
                                                            class="blog-thumnail w-full h-full rounded-[12px] object-cover">
                                                    </div>
                                                </div>
                                            @endforeach
                                        @endif
                ...
                @endif
                @if ($data->title_no_en == 4)
                    <div class="no4-section" id="no{{ $data->title_no_en }}">
                        ...
                        <div component-name="me-blog-no-background-header-table" class="mbnbh-table">
                            {!! @langbind($data, 'desc') !!}
                        </div>
                    </div>
                @endif
                @if ($data->title_no_en == 5)
                    <div class="no5-section" id="no{{ $data->title_no_en }}">
                        <div component-name="me-blog-only-title" class="mbotitle mt-5 mb-[32px]">
                            <h3
                                class="mbo-title helvetica-normal me-body36 font-bold text-darkgray leading-[43.2px]">
                                {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                ...
                @endif
                @if ($data->title_no_en == 6)
                    <div class="no6-section" id="no{{ $data->title_no_en }}">
                        <div component-name="me-blog-only-title" class="mbotitle mt-5 mb-[32px]">
                            <h3
                                class="mbo-title helvetica-normal me-body36 font-bold text-darkgray leading-[43.2px]">
                                {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                        ...
                        <div component-name="me-blog-comparison-table" class="mbc-table">
                            <div class="mbc-container">
                                <div class="mbc-container--outer">
                                    <div class="mbc-container--inner">
                                        @php
                                            $compareProducts = App\Models\Product::whereIn('id', explode(',', $data->product_ids))->get();
                                        @endphp
                                        <table id="blog-table-compare"
                                            class="border-collapse border-spacing-0 overflow-hidden border bg-mee4 rounded-[12px] border-mee4 me-body20 helvetica-normal text-left font-normal text-darkgray leading-[24px] table-rounded-header-bg-style">
                                            <thead>
                                                <tr class="">
                                                    <th class="py-5 px-[32px] font-normal"></th>
                                                    @foreach ($compareProducts as $pdata)
                                                        <th class="py-5 px-[32px] font-normal">
                                                            {{ @langbind($pdata, 'name') }}</th>
                                                    @endforeach
                                                </tr>
                                            </thead>
                                            <tbody>
                                                ...
                                                <tr component-name="me-blog-no-background-header-table-body"
                                                    class="">

                                                    <td component-name="me-blog-no-background-header-td"
                                                        class="py-5 px-[32px] align-top" attr-headers=""></td>


                                                    @foreach ($compareProducts as $pdata)
                                                        <td component-name="me-blog-no-background-header-td"
                                                            class="py-5 px-[32px] align-top" attr-headers=""><img
                                                                src="{{ $pdata->featured_img }}"></td>
                                                    @endforeach
                                                ...
                                                <tr component-name="me-blog-no-background-header-table-body"
                                                    ...
                                                    <td component-name="me-blog-no-background-header-td"
                                                        class="py-5 px-[32px] align-top" attr-headers="">
                                                        @lang('labels.compare.servicec_centres')</td>
                                                    ...
                                                    @foreach ($compareProducts as $pdata)
                                                        ...
                                                        @endphp
                                                        <td component-name="me-blog-no-background-header-td"
                                                            class="py-5 px-[32px] align-top"
                                                            attr-headers="{{ $merchant_name }}">{{ $merchant_name }}</td>
                                                    @endforeach
                                                ...
                                                <tr component-name="me-blog-no-background-header-table-body"
                                                    ...
                                                    <td component-name="me-blog-no-background-header-td"
                                                        class="py-5 px-[32px] align-top" attr-headers="">
                                                        @lang('labels.compare.no_of_items')</td>
                                                ...
                                                <tr component-name="me-blog-no-background-header-table-body"
                                                    ...
                                                    <td component-name="me-blog-no-background-header-td"
                                                        class="py-5 px-[32px] align-top" attr-headers="">Price
                                                    </td>
                                                    ...
                                                    @foreach ($compareProducts as $pdata)
                                                        <td component-name="me-blog-no-background-header-td"
                                                            class="py-5 px-[32px] align-top" attr-headers="">
                                                            HK$ {{ number_format($pdata->original_price,2) }}
                                                        </td>
                                                    @endforeach
                                                ...
                                                <tr component-name="me-blog-no-background-header-table-body"
                                                    ...
                                                    @foreach ($compareProducts as $pdata)
                                                        <td component-name="me-blog-no-background-header-td"
                                                            class="py-5 px-[32px] align-top" attr-headers="">
                                                            <a href="{{ route('product-detail', ['category' => $pdata->category->name_en, 'slug' => $pdata->slug_en]) }}"
                                                                class="mx-auto border border-darkgray rounded-[50px] flex items-center justify-center h-[42px] bg-whitez w-[136px] me-body16 hover:bg-orangeMediq hover:text-whitez hover:border-blueMediq">
                ...
                @endif
                @if ($data->title_no_en == 7)
                    <div class="no7-section" id="no{{ $data->title_no_en }}">
                        <div component-name="me-blog-download-document" class="mbdd-container py-[32px]">
                            <div class="mbdd-container--title">
                                <h3
                                    class="mbvt-title me-body36 helvetica-normal text-darkgray font-bold mb-[32px]">
                                    {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                                </h3>
                            </div>
                            <ul class="mbdd-dowload-section flex items-center jusitfy-start flex-wrap gap-[5px] lg:gap-[16px]">
                                <li class="mb-4">
                                    <a href="{{ Storage::url('blog/' . $data->blog_id . '/' . $data->download1_link) }}"
                                        download
                                        class="w-fit px-5 py-[10px] rounded-[12px] bg-orangeLight flex items-center justify-center text-orangeMediq hover:text-whitez hover:bg-orangeMediq me-body20 helvetica-normal download-blog-link">
                                        <img
                                            src="{{ asset('frontend/img/download-icon.svg') }}"
                                            alt="download icon" class="download-icon mr-[11px] w-[20px] h-[20px]">
                                        {{ @langbind($data, 'download1_name') }}
                                    </a>
                                </li>
                                <li class="mb-4">
                                    <a href="{{ Storage::url('blog/' . $data->blog_id . '/' . $data->download2_link) }}"
                                        download
                                        class="w-fit px-5 py-[10px] rounded-[12px] bg-orangeLight flex items-center justify-center text-orangeMediq hover:text-whitez hover:bg-orangeMediq me-body20 helvetica-normal download-blog-link"><img
                                            src="{{ asset('frontend/img/download-icon.svg') }}"
                                            alt="download icon" class="download-icon mr-[11px] w-[20px] h-[20px]">
                                        {{ @langbind($data, 'download2_name') }}
                                    </a>
                                </li>
                            </ul>
                        </div>
                ...
                @endif
                @if ($data->title_no_en == 8)
                    <div class="no8-section" id="no{{ $data->title_no_en }}">
                        <div component-name="me-blog-form-submission" class="mbfs-container mt-5">

                            <div component-name="me-blog-only-title" class="mbotitle mb-[32px]">
                                <h3
                                    class="mbo-title helvetica-normal me-body36 font-bold text-darkgray leading-[43.2px]">
                                    {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                                </h3>
                            ...
                            <div class="want-to-learn-container rounded-[12px] bg-far p-[40px]">
                                ...
                                <div class="helvetica-normal text-darkgray">
                                    <form id="captcha99Form">
                                        <input name="blog_details_id" type="hidden"
                                        ...
                                        <div>
                                            ...
                                            <div class="pb-[40px]">
                                                <label for="inquiry"
                                                    ...
                                                    <span
                                                        class="ml-5 4xl:ml-[30px] flex items-center me-body18 flex-wrap helvetica-normal">
                                                        @lang('labels.blog.submit_label1')<br>
                ...
                @endif
                @if ($data->title_no_en == 9)
                <div class="no9-section mt-5" id="no{{ $data->title_no_en }}">
                    <div class="product-section">
                      <div component-name="me-blog-only-title" class="mbotitle my-[32px]">
                        <h3 class="mbo-title helvetica-normal me-body36 font-bold text-darkgray leading-[43.2px]"> {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}</h3>
                      </div>
                      <div class="grid-fix">
                        @php
                        $productList = App\Models\Product::whereIn('id', explode(',', $data->product_ids))->get();
                        @endphp
                ...
                @endif
                @if ($data->title_no_en == 10)
                    <div class="coupon-section mt-5" id="no{{ $data->title_no_en }}">
                        <div component-name="me-blog-only-title" class="mbotitle mt-[12px] mb-[32px]">
                            <h3
                                class="mbo-title helvetica-normal me-body36 font-bold text-darkgray leading-[43.2px]">
                                {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                            </h3>
                        ...
                        <div>
                            <div class="mediq-card-section w-full max-w-[600px]">
                                @php
                                    $couponList = App\Models\Coupon::whereIn('id', explode(',', $data->coupon_ids))->get();
                                @endphp
                                @foreach ($couponList as $key => $coupondetails)
                                    ...
                                    <div component-name="me-coupon-new-small-card"
                                        class="small-coupon new-small-coupon" id="new-small-coupon0" data-id="#collect-detail-modal{{ $coupondetails->id }}">
                                            ...
                                            <div class="relative collect">
                                                <div class="custom-voucher-shape gccard rounded-[20px] collect">
                                                    ...
                                                    <div class="gccard-content text-center">
                                                        <div class="cclogo-container show-detail-popup" data-id="collect-detail-modal{{ $coupondetails->id }}">
                                                            <div class="rounded-full">
                                                                <img class="cclogo border-mee4 rounded-full w-[72px] h-[72px]"
                                                                    src="{{ isset($coupondetails->merchant) ? $coupondetails->merchant->icon : '' }}">
                                                            </div>
                                                        </div>
                                                        <div class="gccard-content-body show-detail-popup" data-id="collect-detail-modal{{ $coupondetails->id }}">
                                                        ...
                                                        <div class="gccard-content-bottom show-detail-popup" data-id="collect-detail-modal{{ $coupondetails->id }}">
                                                            <div
                                                                ...
                                                                @else
                                                                    <button
                                                                        class="register-btn clt-button absolute top-1/2 w-full text-center -translate-y-1/2 collect-view-btn text-blueMediq flex justify-center ml-auto mr-auto collect">@lang('labels.coupon.collect')</button>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div
                                                        ...
                                                        <div class="relative bg-title flex justify-start items-center">
                                                            <div class="quantity-logo-container mr-[12px] relative">
                                                                ...
                                                                @endif
                                                                {{-- <div
                                                                    class="label-tag absolute top-0 left-0 new px-[10px] py-[4px] rounded-[4px] 2">
                                                                    @foreach (config('mediq.coupon_status') as $key => $type)
                                                                        @if ($key == $coupondetails->is_new_or_limited)
                                                                            2
                                                                        @endif
                                                        ...
                                                        <button data-id=""
                                                            class="qua-collect-btn w-[131px] font-secondary font-bold me-body16 inline-block py-[10px] px-5 border border-darkgray text-center hover:bg-blueMediq hover:border-blueMediq hover:text-whitez rounded-full">@lang('labels.my_coupon.collected')</button>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="cscard-container">
                                                    ...
                                                    <div class="mb-[27px] last:mb-0">
                                                        ...
                                                        <div class="condes text-darkgray">
                                                            @if($coupondetails->coupon_type===null) 
                                                            {{ date('M d Y', strtotime($coupondetails->start_date)) }}
                                                            to
                                                            {{ date('M d Y', strtotime($coupondetails->end_date)) }}
                                                            @else
                                                            {{$from_date}}
                                                    ...
                                                    <div class="mb-[27px] last:mb-0">
                                                        <h6 class="text-darkgray contitle mb-[10px]">Conditions of Use
                                                        </h6>
                ...
                @endif
                @if ($data->title_no_en == 11)
                    <div id="no{{ $data->title_no_en }}">
                        <div component-name="me-blog-only-title" class="mbotitle mt-5 mb-[32px]">
                            <h3
                                class="mbo-title helvetica-normal me-body36 font-bold text-darkgray leading-[43.2px]">
                                {{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}
                        ...
                        <div class="blog-only-image rounded-[20px]">
                            <div component-name="me-only-image" class="me-only-image">
                                <img src="{{ Storage::url('blog/' . $data->blog_id . '/' . $data->merchant_banner_img) }}"
                                    alt="blog-adv-banner-img" class="object-cover w-full h-full">
                            </div>
                        </div>
                    </div>
                @endif
                @if ($data->title_no_en == 12)
                <div class="no12-section product-should-section" id="no{{ $data->title_no_en }}">
                    <div component-name="me-blog-only-title" class="mbotitle my-[32px]">
                      <h3 class="mbo-title helvetica-normal me-body36 font-bold text-darkgray leading-[43.2px]">{{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}</h3>
                    </div>
                    <div>
                      <div component-name="me-blog-product-should">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center justify-start">
                                <img src="{{ $data->blogSubCategory->img }}" alt=""
                                    class="" width="40">
                                <p
                                    class="ml-[10px] helvetica-normal me-body20 text-darkgray leading-[24px]">
                                    {{ @langbind($data->blogSubCategory, 'name') }}</p>
                            </div>
                            <div>
                                <button type="button"
                                    class="rounded-[50px] bg-whitez border border-darkgray me-body16 helvetica-normal flex items-center justify-center px-[20px] py-[10px] font-bold hover:bg-blueMediq hover:text-whitez hover:border-blueMediq" id="btn_product_list">
                        ...
                        <ul class="mt-[12px] flex-wrap flex items-center justify-start">
                            @php
                                $productIds = App\Models\Product::whereIsPublished(true)->where("sub_category_id",$blogDetails->blogSubCategory->id)->pluck("id")->toArray();
                                $product_key_item_tags = \DB::table('key_item_tags')
                                ->whereNull('key_item_tags.deleted_at')
                                ->where('key_item_tags.is_published', 1)
                                ->whereIn("key_item_tags.id",explode(",",$data->key_item_ids))
                                ->select(
                                    'key_item_tags.id',
                                ...
                                ->whereIn("highlight_tags.id",explode(",",$data->highlight_tag_ids))
                                ->select(
                                    'highlight_tags.id',
                                ...
                                ->get();
                                // $keyItemTags = App\Models\KeyItemTag::whereIsPublished(true)
                                //     ->limit(5)
                                //     ->get();
                            @endphp
                            @foreach ($product_key_item_tags as $kdata)
                                <li class="mr-4 mb-4">
                                    <p
                                        class="helvetica-normal me-body16 text-darkgray rounded-[50px] bg-far px-5 py-[8px] border border-far hover:border-blueMediq hover:text-blueMediq cursor-pointer">
                                        {{ @langbind($kdata, 'name') }}
                                    </p>
                                </li>
                            @endforeach
                ...
                @endif
                @if ($data->title_no_en == 14)
                <div class="no13-section question-section" id="no14">
                    <div component-name="me-popular-questions"
                      class="me-popular-questions-container xl:mt-[32px] mt-[10px] md:mb-[120px] mb-20">
                        <div class="me-popular-questions-content">
                            <h3 class="font-bold me-body32 text-darkgray">{{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}</h3>
                            @php
                                $blogFaqList = \App\Models\BlogDetailsFaq::where("blog_details_id",$data->id)->get();
                            @endphp
                            @foreach($blogFaqList as $bfdata)
                            <div class="faq-div-content">
                            <div class="lg:px-5 px-3 2xl:py-10 py-5 flex justify-between faq-title-bar cursor-pointer ">
                                <h3 class="font-bold me-body26 text-darkgray">{{ @langbind($bfdata, 'question') }}</h3>
                                <img src="{{asset('frontend/img/faq-down.svg')}}" alt="faq-down" />
                            </div>
                            <div class="px-10 faq-answers-content 2xl:mt-[30px] mt-4">
                                {!! @langbind($bfdata, 'answer') !!}
                            </div>
                            </div>
                            @endforeach
                        </div>
            ...
            <div class="last-section">
                <div component-name="me-blog-detail-last-section" class="me-blog-detail-last-section">
                  ...
                  <div class="source-div mb-[32px]">
                    <p class="me-body18 helvetica-normal text-darkgray">@lang('labels.blog.source_of_information'):<a
                        href="{{ $blogDetails->source_title_link }}"
                        class="text-blueMediq">{{ @langbind($blogDetails, 'source_title') }}</a>
                    </p>
                  </div>
        ...
        @php
            $popularArticles = App\Models\Blog::where('category_id', $blogDetails->category_id)
        ...
        <div class="blog-right md:pl-[25px] xl:pl-[45px]">
            ...
            <div component-name="me-summary-card" class="summary-card-container bg-whitez rounded-[20px] shadow-blog p-[32px] mt-[32px] z-[1]">
                ...
                <div class="summary-card-title mb-4">
                    <h6 class="text-darkgray me-body26 helvetica-normal font-bold">Summary</h6>
                </div>
                <div class="summary-links-container pr-2 sm:pr-5">
                    @foreach ($blogDetails->blogDetails as $data)
                    @if($data->title_no_en!=13)
                    <a href="#no{{ $data->title_no_en }}"
                    class="summary-links-item flex items-center justify-start helvetica-normal me-body18 text-darkgray py-[5px] w-full group hover:text-blueMediq">
                    <img src="{{ asset('frontend/img/summary-arrow.svg') }}" alt="summary arrow icon" class="summary-arrow-icon mr-[10px]" />
                    <span>{{ $data->sort_order_no }}. {{ @langbind($data, 'title') }}</span>
                    </a>
                    @endif
                    @endforeach
                </div>
...
<section class="shopping-feature-section" id="related-articles">
    <div component-name="blog-feature-section" class="bfs-container sf-box py-[30px] lg:py-[60px]">
        ...
        <div class="bfs-cardlist flex flex-wrap lg:flex-nowrap items-center three-bc">
            @php
                        ...
                        <div
                            ...
                            <div class="bc-container py-5 px-6 lghtz:px-10 bc-link-card--container--des">
                                ...
                                <p
                                    ...
                                    {{ date('F Y', strtotime($radata->created_at)) }}<img
...
<script>
    ...
    $(document).ready(function() {
        ...
        let product_key_item_tags = <?php echo json_encode($product_key_item_tags->pluck("id")->toArray()); ?>;
        let product_highlight_tags = <?php echo json_encode($product_highlight_tags->pluck("id")->toArray()); ?>;
