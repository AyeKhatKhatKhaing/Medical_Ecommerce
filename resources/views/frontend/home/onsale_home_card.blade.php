    ...
    <div class="mehc-card-left p-[10px] relative">
        <div class="px-[10px] pt-[10px] bg-white mehc-top-card relative">
            <div
                ...
                >
                <div class="discount-circle">-{{(($product->original_price - $product->discount_price) / $product->original_price) * 100}}%</div>
                <div>
                    <img
                        src="{{asset('frontend/img/me-health-care-heart-icon.svg')}}"
            ...
            <div
            class="two-button-container flex justify-between items-center mt-[10px] px-[6px]"
            >
            <button data-preview="preview-modal{{$key}}"
                class="mehc-preview-btn bg-white w-[144px] flex items-center justify-center"
                >
            preview
            </button>
            <button
                class="mehc-compare-btn bg-white w-[64px] flex items-center justify-center"
                >
        ...
        <div class="mehc-card-right px-[10px] pt-[23px] pb-[18px]">
            ...
            @if(count($product->feature_tag_data_arr) > 0)
                @foreach($product->feature_tag_data_arr as $feature)
                <span class="mr-[4px] me-purple px-[10px] py-[3px] text-center rounded-[50px]">{{langbind($feature,'name')}}</span>
                @endforeach
            @endif
                @if(isset($product->free_gift_id))
                <span
                    class="mr-[4px] me-gray px-[10px] py-[3px] text-center rounded-[50px]"
                    >{{$product->freeGift->name_en}} Items</span
                    >
                @endif
                @if(count($product->vaccine_stock_tag_data_arr) > 0)
                @foreach($product->vaccine_stock_tag_data_arr as $stock)
                <span
                    class="mr-[4px] me-gray px-[10px] py-[3px] text-center rounded-[50px]"
                    >{{langbind($stock,'name')}}</span>
                @endforeach
                @endif
                @if(isset($product->number_of_dose))
                <span
                    class="mr-[4px] me-gray px-[10px] py-[3px] text-center rounded-[50px]"
                    >{{$product->number_of_dose}}r</span>
                    @endif

                    @if(count($product->recommend_tag_data_arr) > 0)
                @foreach($product->recommend_tag_data_arr as $recom)
                <span class="mr-[4px] me-gray px-[10px] py-[3px] text-center rounded-[50px]">{{langbind($recom,'name')}}</span>
                @endforeach
                @endif
                @if(count($product->vaccine_factory_tag_data_arr) > 0)
                @foreach($product->vaccine_factory_tag_data_arr as $vaccine)
                <span class="mr-[4px] me-gray px-[10px] py-[3px] text-center rounded-[50px]">{{langbind($vaccine,'name')}}</span>
                @endforeach
                @endif
                @if(count($product->hightlight_tag_data_arr) > 0)
                @foreach($product->hightlight_tag_data_arr as $highlight)
                <span class="mr-[4px] me-gray px-[10px] py-[3px] text-center rounded-[50px]">{{langbind($highlight,'name')}}</span>
                @endforeach
                @endif
            </div>
            ...
            <div class="">
                ...
                <p class="rating-text">
                    <img src="{{asset('frontend/img/health-rating-star.svg')}}" class="inline-block" /> 4.8
