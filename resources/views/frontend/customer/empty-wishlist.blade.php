<div component-name="me-member-no-booking-state" class="me-member-no-booking-state mb-16">
    <div>
      <img src="{{asset('frontend/img/health check 1.svg')}}" alt="health check" class="mx-auto w-auto max-w-[200px]"/>
      <div class="mt-[22px]">
        <p class="font-bold me-body-26 text-darkgray text-center">
          @if($title=='favourite')
          @lang("labels.wishlist.no_favourite_found")
          @elseif($title=='recently-viewed')
          @lang("labels.wishlist.no_recently_viewed_found")
          @else
          @lang("labels.wishlist.no_recently_compared_found")
          @endif
        </p>
        <p class="font-normal me-body-16 text-darkgray mt-3 text-center">
          @if($title=='favourite')
          @lang("labels.wishlist.there_is_no_favourite_found")
          @elseif($title=='recently-viewed')
          @lang("labels.wishlist.there_is_no_recently_viewed_found")
          @else
          @lang("labels.wishlist.there_is_no_recently_compared_found")
          @endif
        </p>
      </div>
